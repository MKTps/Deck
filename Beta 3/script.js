...
const introVideo = document.getElementById('intro-video'); // âœ… mantenido aquÃ­ para evitar referencias antes de tiempo
let isMuted = true;
...
function initSwiper() {
    ...
    animateSlideIn(0);
            ...
            // Feature-card modals: one per service container (independent)
            ...
            function buildSlidesForCard(card){
                const slideEls = card.querySelectorAll('.feature-slide');
                if(slideEls.length){
                    return Array.from(slideEls).map(el => ({
                        heading: (el.getAttribute('data-heading') || '').trim(),
                ...
                const title = titleEl ? titleEl.textContent.trim() : 'Detalle';
                const desc = pEl ? pEl.textContent.trim() : '';

                return [
                    { heading: title, body: desc || 'Por que elegirnos.' },
                    { heading: title + ' - Valor', body: 'Beneficios clave y enfoque tecnico.' },
                    { heading: title + ' - Casos', body: 'Casos de uso y siguientes pasos.' }
                ];
            }
            function animateModalSlideIn(modal){
                const activeSlide = modal.querySelector('.swiper-slide-active');
                if(!activeSlide) return;
                const heading = activeSlide.querySelector('h2');
                const body = activeSlide.querySelector('p');
                if(heading){
                    gsap.fromTo(heading, { opacity: 0, x: -60 }, { opacity: 1, x: 0, duration: 0.6, ease: 'power3.out' });
                }
                if(body){
                    gsap.fromTo(body, { opacity: 0, x: 60 }, { opacity: 1, x: 0, duration: 0.7, ease: 'power3.out', delay: 0.1 });
                }
            }

            // Setup per-modal parallax driven by the modal's internal scroll
            function setupModalParallax(modal){
                try{
                    if(!window.gsap || !window.ScrollTrigger) return;
                    if(window.innerWidth <= 768) return; // disable on small screens

                    const scroller = modal.querySelector('.feature-modal__content');
                    if(!scroller) return;

                    // ensure previous triggers cleaned
                    teardownModalParallax(modal);

                    modal.__modalParallaxTriggers = [];


                    const slideInners = modal.querySelectorAll('.swiper-slide .slide-inner');
                    slideInners.forEach(inner => {
                        // create a scroll-linked tween that moves the inner content slightly (parallax)
                        ...
                        if(tween && tween.scrollTrigger) modal.__modalParallaxTriggers.push(tween.scrollTrigger);
                    });
                }catch(err){ console.warn('Error setting up modal parallax', err); }
            }

            function teardownModalParallax(modal){
                try{
                    if(!modal || !modal.__modalParallaxTriggers) return;
                    modal.__modalParallaxTriggers.forEach(st => { try{ st.kill(); }catch(e){} });
                    modal.__modalParallaxTriggers = null;
                }catch(err){ console.warn('Error tearing down modal parallax', err); }
            }
            ...
            function openFeatureModal(card){
                ...
                const wrapper = featureModal.querySelector('.swiper-wrapper');
                wrapper.innerHTML = '';
                
                slides.forEach(s => {
                    const slide = document.createElement('div');
                    slide.className = 'swiper-slide';
                    slide.innerHTML = `<div class="slide-inner"><h2>${s.heading}</h2><p>${s.body}</p></div>`;
                    wrapper.appendChild(slide);
                });

                // Force Swiper update after slides are injected
                if(featureModal.__featureModalSwiper){
                    try{ featureModal.__featureModalSwiper.update(); featureModal.__featureModalSwiper.slideTo(0); }catch(e){}
                }

                featureModal.setAttribute('aria-hidden','false');
                document.body.classList.add('no-scroll');

                // Animate with GSAP (slowed down for better visibility)
                gsap.fromTo(featureModal, { opacity: 0 }, { opacity: 1, duration: 0.5, ease: 'power2.out' });
                gsap.fromTo(featureModal.querySelector('.feature-modal__panel'), { transform: 'scale(0.92)', opacity: 0 }, { transform: 'scale(1)', opacity: 1, duration: 0.8, ease: 'cubic-bezier(0.2, 0.8, 0.2, 1)' });

                // init or re-init swiper inside this modal only
                const swiperEl = featureModal.querySelector('.feature-modal-swiper');
                const nextEl = featureModal.querySelector('.swiper-button-next');
                const prevEl = featureModal.querySelector('.swiper-button-prev');
                const paginationEl = featureModal.querySelector('.swiper-pagination');
                if(featureModal.__featureModalSwiper){
                    try{ featureModal.__featureModalSwiper.update(); featureModal.__featureModalSwiper.slideTo(0); }catch(e){}
                } else if(swiperEl) {
                    featureModal.__featureModalSwiper = new Swiper(swiperEl, {
                        loop: false,
                        speed: 600,
                        autoHeight: true,
                        ...
                        on: {
                            init: function(){
                                animateModalSlideIn(featureModal);
                            },
                            slideChangeTransitionEnd: function(){
                                animateModalSlideIn(featureModal);
                            }
                    ...
                    try{ featureModal.__featureModalSwiper.init(); }catch(e){}
                }

                // Animate active slide content on open
                animateModalSlideIn(featureModal);

                // Setup modal-internal parallax (based on modal scroll)
                try{ setupModalParallax(featureModal); }catch(e){/* ignore */}
            }

            function closeFeatureModal(featureModal){
                if(!featureModal) return;
                
                // Animate close with GSAP (zoom out + fade out)
                gsap.to(featureModal, { opacity: 0, duration: 0.5, ease: 'power2.in' });
                gsap.to(featureModal.querySelector('.feature-modal__panel'), { transform: 'scale(0.92)', opacity: 0, duration: 0.8, ease: 'cubic-bezier(0.8, 0.2, 0.8, 0.8)', onComplete: () => {
                    featureModal.setAttribute('aria-hidden','true');
                    document.body.classList.remove('no-scroll');
                    try{ teardownModalParallax(featureModal); }catch(e){}
                    if(featureModal.__featureModalSwiper){
                        try{ featureModal.__featureModalSwiper.destroy(true,true); }catch(e){}
                        featureModal.__featureModalSwiper = null;
                    }
                }});
            }

            // Attach listeners to all existing and future .feature-card elements
            function attachFeatureCardListeners(){
                const cards = document.querySelectorAll('.feature-card');
                cards.forEach(card => {
                    // prevent duplicate listeners
                    if(card.__fsm_listener_attached) return;
                    card.addEventListener('click', (e)=>{ openFeatureModal(card); });
                    card.__fsm_listener_attached = true;
                });
            }
            ...
            document.addEventListener('keydown', (e)=>{
                if(e.key === 'Escape'){
                    document.querySelectorAll('.feature-modal[aria-hidden=\"false\"]').forEach(modal => {
                        closeFeatureModal(modal);
                    });
                }
            });
