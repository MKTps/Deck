...
function initSwiper() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (sliderSection && sliderSection.classList.contains('active')) {
            ...
            function animateModalActiveState(tabContent, options = {}){
                ...
                const animTargets = textOnly
                    ? [activeSlide, heading, text].filter(Boolean)
                    : [leftColumn, rightColumn, activeSlide, heading, text, image, slideNav, slideCounter].filter(Boolean);
                if(animTargets.length > 0){
                    gsap.killTweensOf(animTargets);
                }

                if(!textOnly && leftColumn){
            ...
            function navigateSlides(container, direction, tabContent){
                ...
                animateModalActiveState(tabContent, { textOnly: true });
            }
...
(function(){
    ...
    function detectSafari(){
        ...
        const isOtherWebKitBrowser = /CriOS|FxiOS|EdgiOS|OPiOS|Chrome|Android/i.test(ua);
        return isSafariEngine && !isOtherWebKitBrowser;
    }
    ...
    if(window.visualViewport){
        ...
        window.visualViewport.addEventListener('scroll', setAppHeightVar, { passive: true });
    }
})();
